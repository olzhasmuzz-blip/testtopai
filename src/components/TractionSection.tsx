...
import { translations, tractionMetrics, userEngagementStats, aiEngineStats, testimonials, liveFeedActivities } from '../data/content';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-7 mb-12">
          <div className="bg-white p-7 rounded-3xl border border-slate-200 shadow-sm reveal-left">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-5">
              <h3 className="font-extrabold text-lg text-slate-900 font-display">{lang === 'ru' ? 'Вовлечённость пользователей' : 'User Engagement'}</h3>
              <span className="text-xs font-mono font-bold text-orange-600 bg-orange-50 px-2.5 py-1 rounded-lg">Last 30 days</span>
            </div>
          ...
          <div className="bg-white p-7 rounded-3xl border border-slate-200 shadow-sm reveal-right">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-5">
              <h3 className="font-extrabold text-lg text-slate-900 font-display">{lang === 'ru' ? 'Производительность AI' : 'AI Engine Performance'}</h3>
        ...
        <div className="bg-slate-900 text-white rounded-3xl p-7 sm:p-9 shadow-xl reveal" style={{ transitionDelay: '.2s' }}>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-7 border-b border-slate-800 pb-5">
            <div>
              <span className="flex items-center gap-1.5 text-xs font-mono font-bold text-orange-400 uppercase mb-1">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />Live Student Network
              </span>
              <h3 className="text-xl sm:text-2xl font-extrabold font-display">{lang === 'ru' ? 'Истории успеха выпускников TestTop' : 'Verified Candidate Admissions'}</h3>
            </div>
          ...
          <div className="flex items-center justify-center gap-3">
            <button onClick={() => setTIdx(i => (i - 1 + pages) % pages)} className="w-9 h-9 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center hover:bg-slate-700 transition-all">
            ...
            <button onClick={() => setTIdx(i => (i + 1) % pages)} className="w-9 h-9 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center hover:bg-slate-700 transition-all">
