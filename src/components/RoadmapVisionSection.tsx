...
import { Sparkles, Globe2, ChevronDown, ArrowRight, Send, Award, Newspaper, Compass, Check, Lock } from 'lucide-react';
...
export const RoadmapVisionSection: React.FC<RoadmapVisionSectionProps> = ({ lang, onOpenTestModal }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => window.open('https://testtop.app/', '_blank'), 1200);
  };
    ...
    <section id="roadmap" ref={sectionRef} className="py-20 md:py-28 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          ...
          <h2 className="reveal text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 font-display" style={{ transitionDelay: '.08s' }}>{t.roadmap.title}</h2>
          <p className="reveal text-slate-500 text-base" style={{ transitionDelay: '.14s' }}>{t.roadmap.subtitle}</p>
        </div>
          ...
          <div className="lg:col-span-6 bg-slate-50 p-7 rounded-3xl border border-slate-200 text-left flex flex-col justify-between reveal-left">
            <div>
              <div className="flex items-center justify-between mb-5">
                <div className="w-10 h-10 rounded-xl brand-gradient text-white flex items-center justify-center shadow-md brand-glow">
                  <Award className="w-5 h-5" />
                </div>
                <span className="px-2.5 py-1 bg-emerald-100 text-emerald-800 text-xs font-bold rounded-lg font-mono">Proven Track Record</span>
              </div>
              <div className="flex items-center gap-4 mb-5 p-4 rounded-2xl bg-white border border-slate-200 shadow-sm">
                ...
                <div>
                  <h4 className="font-extrabold text-sm text-slate-900 font-display">{lang === 'ru' ? 'Основатели TestTop & Fluentme' : 'TestTop & Fluentme Founders'}</h4>
                  <p className="text-xs text-slate-500">{lang === 'ru' ? 'EdTech ветераны, Cambridge & ITMO ML Lab' : 'EdTech Veterans, 10+ yrs NLP & ML'}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 my-6 text-center">
                {[{ v: '2019', l: 'Founded', c: 'text-slate-900' }, { v: '2022', l: 'Exited', c: 'text-emerald-600' }, { v: '5,000+', l: 'MAU', c: 'text-orange-600' }].map(s => (
            ...
            <div className="p-3.5 rounded-2xl bg-white border border-slate-200 flex items-center justify-between gap-3 text-xs text-slate-600">
              ...
              <span className="font-bold text-orange-600 font-mono shrink-0">Forbes 2022</span>
            </div>
          </div>
                  ...
                  return (
                    <div key={i} onClick={() => setActiveMilestone(i)}
                      className={`relative p-4 rounded-2xl border transition-all duration-300 cursor-pointer
                        ${isActive ? 'bg-orange-50 border-orange-200 shadow-sm' : 'bg-slate-50/80 border-slate-100 hover:bg-slate-100/50'}`}>
                      <div className={`absolute -left-[1.65rem] top-4 w-5 h-5 rounded-full border-2 flex items-center justify-center text-[10px] font-black
                        ${m.status === 'completed' ? 'border-emerald-500 bg-emerald-500 text-white'
                          : m.status === 'active' ? 'text-white border-none'
                          : 'border-slate-300 bg-white text-slate-400'}`}
                        style={m.status === 'active' ? { background: 'linear-gradient(135deg,#F97316,#F43F5E)' } : {}}>
                      ...
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded-md ${m.status === 'active' ? 'brand-gradient text-white' : 'bg-slate-200 text-slate-600'}`}>
                          {m.period}
                        </span>
                      </div>
                      {isActive && <p className="text-xs text-slate-600 leading-relaxed mt-1 slide-up">{m.description}</p>}
                      <span className={`inline-block mt-1.5 text-[10px] font-mono font-bold px-2 py-0.5 rounded-md ${m.status === 'completed' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                        {m.metrics}
                      </span>
                    </div>
                  );
        ...
        <div className="bg-slate-900 text-white rounded-3xl p-8 sm:p-12 border border-slate-800 shadow-2xl mb-16 relative overflow-hidden reveal" style={{ transitionDelay: '.18s' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">
            <div className="lg:col-span-8 space-y-3">
              ...
              <h3 className="text-xl sm:text-2xl md:text-3xl font-extrabold font-display leading-snug">{t.roadmap.visionText}</h3>
            </div>
        ...
        {/* CTA lead capture */}
        <div className="p-9 sm:p-12 rounded-3xl border-2 border-orange-300 shadow-xl mb-16 text-center max-w-4xl mx-auto reveal"
          style={{ background: 'linear-gradient(135deg, #FFF7ED 0%, #FFF1F2 100%)', transitionDelay: '.22s' }}>
          <div className="w-14 h-14 mx-auto rounded-2xl brand-gradient text-white flex items-center justify-center shadow-lg brand-glow mb-5 animate-float-slow">
          ...
          <form onSubmit={handleSubmit} className="max-w-md mx-auto flex flex-col sm:flex-row gap-2">
            <input type="text" required value={email} onChange={e => setEmail(e.target.value)}
              ...
              className="flex-1 px-4 py-3 text-sm rounded-xl bg-white border border-slate-300 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 outline-none shadow-sm text-slate-800 transition-all" />
            <button type="submit"
              className="px-6 py-3 brand-gradient text-white font-bold text-sm rounded-xl shadow-md brand-glow flex items-center justify-center gap-2 cursor-pointer transition-all hover:scale-[1.02]">
        ...
        <div className="max-w-3xl mx-auto reveal" style={{ transitionDelay: '.26s' }}>
          ...
          <div className="space-y-3">
            {faqs.map((faq, i) => {
              ...
              return (
                <div key={i} className={`rounded-2xl border overflow-hidden transition-all duration-300 ${isOpen ? 'border-orange-200 bg-white shadow-sm' : 'border-slate-200 bg-slate-50/50 hover:border-slate-300'}`}>
                  <button onClick={() => setOpenFaq(isOpen ? null : i)}
                    className="w-full p-5 flex items-center justify-between text-left font-bold text-slate-900 text-sm cursor-pointer hover:text-orange-600 transition-colors">
                    <span>{faq.question}</span>
                    <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform duration-300 shrink-0 ml-3 ${isOpen ? 'rotate-180 text-orange-500' : ''}`} />
                  </button>
                  <div className={`overflow-hidden transition-all duration-300 ${isOpen ? 'max-h-96' : 'max-h-0'}`}>
                    <div className="px-5 pb-5 text-sm text-slate-600 leading-relaxed border-t border-slate-100 pt-4 bg-white">{faq.answer}</div>
                  </div>
                </div>
              );
            })}
