...
import {
  RefreshCw, Cpu, UserCheck, FileText,
...
const STEP_ICONS: Record<number, React.ReactNode> = {
  1: <UserCheck className="w-6 h-6" />,
  2: <Cpu className="w-6 h-6" />,
  3: <FileText className="w-6 h-6" />,
  4: <RefreshCw className="w-6 h-6" />,
};

const STEP_COLORS = ['#6366F1', '#10B981', '#F59E0B', '#8B5CF6'];
            ...
            {loopSteps.map((s, i) => {
              ...
              return (
                <React.Fragment key={s.stepNumber}>
                  <button
                    onClick={() => { setActive(s.stepNumber); setPlaying(false); setProgress(0); }}
                    ...
                    <div className={`w-11 h-11 rounded-full flex items-center justify-center border-2 transition-all duration-300 font-bold text-sm
                      ${isActive
                        ? 'border-indigo-600 bg-indigo-600 text-white shadow-lg shadow-indigo-600/30 scale-110'
                        : isDone
                          ? 'border-emerald-500 bg-emerald-500 text-white'
                          : 'border-slate-300 bg-white text-slate-400 group-hover:border-indigo-300'}`}
                      style={isActive ? { boxShadow: `0 0 0 4px rgba(99,102,241,0.2)` } : {}}>
                      {isDone ? '✓' : s.stepNumber}
                    </div>
                  ...
                  {i < loopSteps.length - 1 && (
                    <div className="flex-1 mx-2 h-1.5 rounded-full overflow-hidden bg-slate-200">
                      <div className={`h-full rounded-full transition-all duration-700 ${isDone ? 'bg-emerald-500' : 'bg-slate-200'}`}
              ...
              <div className="lg:col-span-7 p-7 sm:p-10 text-white flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="text-2xl sm:text-3xl font-black font-display mb-2" key={`title-${step.stepNumber}`}
                    style={{ animation: 'slide-up .4s cubic-bezier(.16,1,.3,1)' }}>
                    {step.subtitle}
                  </h3>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
                  {[
                    ...
                    { label: 'Precision', val: '98.8%', color: 'text-indigo-400' },
            ...
            return (
              <button key={s.stepNumber} onClick={() => { setActive(s.stepNumber); setPlaying(false); setProgress(0); }}
                className={`p-5 rounded-2xl border text-left transition-all duration-300 cursor-pointer card-hover
                  ${isActive ? 'bg-slate-900 border-slate-700 shadow-xl text-white' : 'bg-white border-slate-200 hover:border-indigo-200 text-slate-800'}`}>
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 transition-all
                  ${isActive ? 'text-white shadow-lg' : 'bg-indigo-50 text-indigo-600'}`}
        ...
        <div className="reveal bg-white rounded-3xl border border-slate-200 p-7 sm:p-8 shadow-sm" style={{ transitionDelay: '.26s' }}>
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
            ...
            <div className="lg:col-span-4 flex items-center gap-8 pt-4 lg:pt-0 border-t lg:border-t-0 lg:border-l border-slate-200 lg:pl-8">
              ...
              <div>
                <span className="block text-4xl font-black text-indigo-600 font-display">{t.solution.availabilityBadge}</span>
                <span className="text-xs text-slate-500 font-mono">{t.solution.availabilityLabel}</span>
              </div>
            </div>
          </div>
          <div className="mt-5 p-4 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-between gap-3 flex-wrap">
