...
const SEVERITY_COLORS: Record<number, string> = {
  95: '#EF4444', 90: '#F97316', 85: '#F59E0B', 80: '#EAB308', 75: '#84CC16',
};
...
export const ProblemSection: React.FC<ProblemSectionProps> = ({ lang }) => {
  ...
  const iconMap: Record<string, React.ReactNode> = {
    tutors: <DollarSign className="w-5 h-5" />, static: <BookOpen className="w-5 h-5" />,
    feedback: <Clock className="w-5 h-5" />, eval: <MicOff className="w-5 h-5" />, offline: <WifiOff className="w-5 h-5" />,
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          <div className="reveal inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-rose-50 border border-rose-200 text-rose-700 text-xs font-bold uppercase tracking-wider">
            <AlertTriangle className="w-3.5 h-3.5" />{t.problem.sectionTag}
          </div>
          ...
          <p className="reveal text-slate-500 text-base leading-relaxed" style={{ transitionDelay: '.14s' }}>{t.problem.subtitle}</p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div className="lg:col-span-5 reveal-left">
              <div className="relative rounded-3xl overflow-hidden shadow-2xl h-[420px] group">
                <img key={animKey} src={PROBLEM_PHOTOS[active.id]} alt={active.title}
                  className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
                  style={{ animation: 'slide-up .5s cubic-bezier(.16,1,.3,1)' }} referrerPolicy="no-referrer" />
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2 py-0.5 rounded-md bg-rose-500/80 text-white text-[10px] font-bold font-mono uppercase">{active.flaw}</span>
                  </div>
                ...
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                  {items.map((_, i) => (
                    <button key={i} onClick={() => goTo(i)} className={`rounded-full transition-all ${i === activeIdx ? 'w-5 h-2 bg-white' : 'w-2 h-2 bg-white/40'}`} />
                  ))}
                </div>
              </div>
        ...
        {mode === 'calc' && (
          <div className="max-w-3xl mx-auto reveal slide-up">
            <div className="bg-slate-50 border border-slate-200 rounded-3xl p-8 space-y-8 shadow-sm">
              <div>
                <h3 className="text-xl font-black text-slate-900 font-display">{lang === 'ru' ? 'Калькулятор личной экономии' : 'Personal ROI Calculator'}</h3>
                <p className="text-xs text-slate-500 mt-1 font-mono">{lang === 'ru' ? 'Сравните стоимость репетиторов с TestTop AI' : 'Compare private tutor costs vs TestTop AI'}</p>
              </div>
