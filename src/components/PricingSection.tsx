          ...
          <div className="reveal flex flex-col sm:flex-row items-center justify-center gap-4 pt-2" style={{ transitionDelay: '.18s' }}>
            ...
            {tab === 'subscription' && (
              <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-2xl border border-slate-200 shadow-sm">
                <span className={`text-xs font-semibold ${!isAnnual ? 'text-indigo-600 font-bold' : 'text-slate-400'}`}>
                  {t.pricing.billingMonthly}
                </span>
                <button onClick={() => setIsAnnual(!isAnnual)}
                  className={`relative w-11 h-6 rounded-full transition-colors cursor-pointer ${isAnnual ? 'bg-indigo-600' : 'bg-slate-300'}`}>
                ...
                <span className={`text-xs font-bold flex items-center gap-1 ${isAnnual ? 'text-indigo-600' : 'text-slate-400'}`}>
              ...
              return (
                <div key={plan.id}
                  className={`rounded-3xl p-7 flex flex-col justify-between text-left transition-all relative card-hover
                    ${plan.popular
                      ? 'bg-white border-2 border-indigo-600 shadow-2xl shadow-indigo-600/15 md:-translate-y-3'
                      : 'bg-white border border-slate-200 shadow-sm hover:border-slate-300'}`}>
                  {plan.popular && (
                    <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 bg-indigo-600 text-white text-[11px] font-extrabold uppercase tracking-wider rounded-full shadow-md shadow-indigo-600/30 flex items-center gap-1.5 font-mono">
                      <Sparkles className="w-3 h-3" />
                      {lang === 'ru' ? 'Выбор 85% студентов' : 'Most Popular'}
                    </div>
                  )}
                  <div>
                    ...
                    <div className="mt-6 mb-6 pb-6 border-b border-slate-100 flex items-baseline gap-1">
                      ...
                      {isAnnual && (
                        <span className="ml-2 text-[10px] text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md font-mono font-bold">billed annually</span>
                      )}
                    </div>
              ...
              <div key={i} className="bg-white p-7 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-between text-left card-hover">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="px-2.5 py-0.5 text-[11px] font-bold uppercase text-indigo-600 bg-indigo-50 border border-indigo-200 rounded-md font-mono">{mp.type}</span>
                ...
                <button onClick={onOpenTestModal}
                  className="w-full py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-sm font-bold cursor-pointer transition-all">
          ...
          <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm mb-14 slide-up reveal">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              <div className="lg:col-span-8 space-y-4">
                <span className="px-3 py-1 bg-blue-50 text-blue-700 text-xs font-bold rounded-full font-mono">
                  Institutional Partners & Schools
                </span>
                <h3 className="text-2xl sm:text-3xl font-black text-slate-900 font-display">
                  {lang === 'ru' ? 'AI-инфраструктура для языковых школ' : 'AI Infrastructure for Language Centers'}
                </h3>
                <p className="text-sm text-slate-600 leading-relaxed">
                  {lang === 'ru'
                    ? 'Подключите студентов к единой платформе и автоматизируйте 100% рутинной проверки эссе и домашних заданий.'
                    : 'Connect student cohorts to a unified dashboard. Automate 100% of repetitive homework and essay grading.'}
                </p>
              ...
              <div className="lg:col-span-4">
                <button onClick={onOpenTestModal}
                  className="w-full py-4 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm shadow-lg shadow-indigo-600/20 cursor-pointer transition-all hover:scale-[1.02] flex items-center justify-center gap-2">
                  <Building className="w-5 h-5" />
                  {lang === 'ru' ? 'Запросить демо для школы' : 'Request School Demo'}
                </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 reveal" style={{ transitionDelay: '.26s' }}>
          <div className="bg-white p-7 rounded-3xl border border-slate-200 shadow-sm text-left">
            ...
            <div className="grid grid-cols-3 gap-4 text-center mb-4">
              {[
                { val: '$0.01', label: 'Marginal Cost', color: 'text-slate-900' },
                { val: '85%',   label: 'Gross Margin',  color: 'text-emerald-600' },
                { val: '100x',  label: 'Scalability',   color: 'text-indigo-600' },
              ].map(s => (
          ...
          <div className="bg-white p-7 rounded-3xl border border-slate-200 shadow-sm text-left">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={s.title} className="p-3.5 rounded-xl bg-slate-50 border border-slate-100 hover:border-indigo-200 transition-colors">
